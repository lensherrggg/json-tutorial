...
static void test_parse_array() {
    ...
    for (i = 0; i < 4; i++) {
        lept_value* w = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(w));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(w));
        for (j = 0; j < i; j++) {
            lept_value* x = lept_get_array_element(w, j);
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(x));
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(x));
        }
    }
    lept_free(&v);
}
