...
static int lept_parse_object(lept_context* c, lept_value* v) {
    ...
    for (;;) {
        ...
        m.k = (char*)malloc(sizeof(char) * m.klen + 1);
        ...
        if (*c->json == '}') {
            ...
            v->type = LEPT_OBJECT;
            v->u.o.size = size;
            size_t mem_size = sizeof(lept_member) * size;
            v->u.o.m = (lept_member*)malloc(mem_size);
            memcpy(v->u.o.m, lept_context_pop(c, mem_size), mem_size);
            return LEPT_PARSE_OK;
        } else if (*c->json == ',') {
            c->json++;
            lept_parse_whitespace(c);
